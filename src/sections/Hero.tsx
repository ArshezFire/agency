...
import { ArrowUpRight, BadgeCheck, ChevronDown, ChevronRight, Flame, Zap } from "lucide-react"
...
export function Hero() {
  ...
  return (
    <section
      id="inicio"
      className="relative mx-auto max-w-7xl px-4 pt-24 pb-4 sm:px-6 sm:pt-28 sm:pb-16 lg:flex lg:min-h-svh lg:flex-col lg:justify-center lg:pb-10"
    >
      ...
      <div className="grid gap-9 md:grid-cols-2 md:items-center md:gap-8 lg:grid-cols-[1.12fr_0.95fr_0.78fr] xl:gap-12">
        ...
        <div className="contents md:flex md:flex-col md:gap-10 lg:gap-9">
          <Reveal className="order-1 flex flex-col gap-5 sm:gap-6">
            <div className="flex flex-wrap items-center gap-2">
              <a
                href="#agencia"
                className="glass-strong inline-flex h-9 items-center gap-1.5 rounded-full pr-3 pl-2 text-[13px] font-bold text-ink transition hover:bg-white/90"
              >
                <BadgeCheck className="size-4 text-brand-violet" />
                Agencia oficial
                <ChevronRight className="size-3.5 text-ink-soft" />
              </a>
              <Chip className="glass-strong h-9 gap-1.5 rounded-full px-3 text-[13px] font-extrabold text-ink">
                <span className="size-2 rounded-full bg-gradient-brand" />
                <Chip.Label>Solo +18</Chip.Label>
              </Chip>
            </div>

            <h1 className="text-[2.6rem] leading-[1.02] font-black tracking-[-0.035em] text-ink sm:text-6xl xl:text-[4.4rem]">
              Talento real, contenido <span className="text-gradient">exclusivo</span>
            </h1>

            <p className="max-w-md text-[17px] leading-relaxed font-medium text-ink-soft">
            ...
            <div className="flex flex-wrap items-center gap-3 lg:hidden">
              ...
              <a
                href="#modelos"
                className="glass-strong inline-flex h-13 items-center gap-2 rounded-full px-5 text-[15px] font-extrabold text-ink"
              >
                Ver modelos
        ...
        <div className="hidden flex-col gap-9 lg:flex">
          ...
          <Reveal delay={0.26} className="flex flex-col gap-5">
            ...
            <div>
              <p className="text-[3.25rem] leading-none font-black tracking-[-0.04em] text-ink">
                {bigStat.value}
                <span className="text-ink/25">+</span>
              </p>
              <p className="mt-1 text-sm font-semibold text-ink-soft">{bigStat.label.toLowerCase()}</p>
            </div>
          </Reveal>
      ...
      <a
        href="#agencia"
        className="absolute right-6 bottom-2 hidden items-center gap-1.5 text-sm font-bold text-ink-soft transition hover:text-ink lg:flex"
      >
        Desliza
...
function TrustRow() {
  ...
  return (
    <div className="flex items-center gap-5">
      <div>
        <p className="mb-2 text-xs font-bold text-ink-soft">Nuestras modelos</p>
        <AvatarGroup overlap="ring" className="[--avatar-group-overlap:0.7rem]">
          ...
          <AvatarGroup.Count className="size-11 rounded-full bg-white text-xs font-extrabold text-ink">
            {site.stats[0].value}
          </AvatarGroup.Count>
        </AvatarGroup>
      </div>
      ...
      <div>
        ...
        <p className="mt-0.5 text-xs font-medium text-ink-soft">Perfiles reales y mayores de edad</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="w-full max-w-md">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-extrabold text-ink">Nuestra comunidad</p>
        <a href="#agencia" className="text-sm font-semibold text-ink-soft transition hover:text-ink">
          Ver más
        </a>
      </div>
      <div className="grid grid-cols-2 items-end gap-3">
        ...
        <div className="rounded-[26px] bg-linear-to-br from-brand-plum to-brand-violet p-1 pt-0 shadow-float">
          <p className="flex h-8 items-center gap-1.5 px-3 text-xs font-bold text-white/90">
            <Flame className="size-3.5" />
            Lo más buscado
          </p>
          <div className="rounded-[22px] bg-white/95 p-4">
            <p className="text-xs font-semibold text-ink-soft">{first.label}</p>
            <p className="mt-1 text-[28px] leading-none font-black tracking-tight text-ink">{first.value}</p>
        ...
        <div className="glass rounded-[26px] p-4">
          <p className="text-xs font-semibold text-ink-soft">{third.label}</p>
          <p className="mt-1 text-[28px] leading-none font-black tracking-tight text-ink">{third.value}</p>
...
function FeaturedPortrait({ model }: { model: Model }) {
  return (
    <div className="relative mx-auto w-full max-w-[440px]">
      <div className="relative aspect-[5/6] overflow-hidden rounded-[36px] shadow-float ring-1 ring-white/70 md:aspect-[3/4] lg:aspect-[10/14]">
        <ModelPhoto model={model} index={0} eager />
        <div aria-hidden="true" className="absolute inset-x-0 top-0 h-32 bg-linear-to-b from-black/25 to-transparent" />
        <span className="glass-dark absolute top-4 left-4 inline-flex items-center gap-1.5 rounded-full py-1.5 pr-3.5 pl-2.5 text-xs font-extrabold text-white">
          <Flame className="size-3.5" />
          Top de la semana
        </span>
        ...
        <div className="glass-photo absolute inset-x-3 bottom-3 flex items-end justify-between gap-3 rounded-[28px] p-5">
          <div className="min-w-0">
            <p className="truncate text-[13px] font-semibold text-ink-soft">{model.tagline}</p>
            <p className="mt-0.5 truncate text-[34px] leading-none font-black tracking-[-0.03em] text-ink">
              {model.name}
            </p>
            <p className="mt-1.5 text-[13px] font-semibold text-ink-soft">Contenido nuevo cada día</p>
          </div>
      ...
      {/* Insignia flotante */}
      <div className="glass-strong absolute top-[38%] -left-3 hidden animate-float items-center gap-2 rounded-full py-1.5 pr-3.5 pl-1.5 text-xs font-extrabold text-ink sm:flex motion-reduce:animate-none">
  ...
  return (
    <>
      <div className="relative aspect-[5/4] overflow-hidden rounded-[22px]">
        ...
        <span className="glass-photo absolute top-3 left-3 rounded-full px-3 py-1 text-xs font-extrabold text-ink">
          Nueva
        </span>
      </div>
      <div className="flex items-end justify-between gap-2 px-2.5 pt-3 pb-1.5">
        <div className="min-w-0">
          <p className="text-xs font-semibold text-ink-soft">Recién llegada</p>
          <p className="truncate text-xl font-black tracking-tight text-ink">{model.name}</p>
        </div>
        <a
          ...
          className="grid size-10 shrink-0 place-items-center rounded-full bg-white/80 text-ink ring-1 ring-ink/5 transition hover:bg-white"
        >
  ...
  return (
    ...
    >
      <div className="mr-auto">
        <p className="text-sm font-extrabold text-ink">Modelos</p>
      ...
      {visible.map((modelIndex, slot) => (
        ...
        >
          ...
          >
            <ModelPhoto model={models[modelIndex]} index={modelIndex} initial />
          </span>
        </span>
      ))}
    </a>
